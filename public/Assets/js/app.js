...
async function loadAudio() {
  try {
    ...
    audio.enabled = false;
    console.log("Audio Track Added", audio);
    ...
    recognition.onresult = (event) => {
      ...
      if (recognizedText.trim()) {
        // $('#speechAudio').html(
        //   `<p style='width:100%;'>${recognizedText}</p>`
        // );
        $('#speechAudio').html(`<p style='width:100%;'>${recognizedText}</p>`).show();
    ...
    console.log("Error accessing mic", e);
  }
}
...
function eventProcess() {
  $("#miceMuteUnmute").on("click", async function () {
    ...
    if (isAudioMute) {
      // Unmute audio and start speech recognition
      audio.enabled = true;
      $(this).html(
      ...
      // $('#speechAudio').html('');
      // Clear displayed text and hide the container
      $('#speechAudio').html('').hide();
...
// async function loadAudio() {
//   try {
//     // Access user's microphone for audio input
//     var astream = await navigator.mediaDevices.getUserMedia({
//       video: false,
//       audio: true,
//     });
//     audio = astream.getAudioTracks()[0];
//     audio.enabled = false; // Disable audio track (if not needed for transmission)
//     console.log("Audio Track Added", audio);
...
//       // Display recognized text for the current user
//       document.getElementById("transcription").innerHTML += `<p>You: ${recognizedText}</p>`;

//       // Send recognized text to the server for broadcasting
//       // socket.emit("speech_to_text", { text: recognizedText });
//     };
...
//     // Listen for text broadcasted by other users
//     // socket.on("broadcast_text", (data) => {
//     //   document.getElementById("transcription").innerHTML += `<p>${data.user}: ${data.text}</p>`;
//     // });
//   } catch (e) {
